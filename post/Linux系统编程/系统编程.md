...
**目录项**主要包括了文件名和索引节点号，索引节点号是指向索引节点表( system inode table )中对应的索引节点的。

硬链接创建的就是目录项，unlink释放的也是目录项。

**索引节点**就是文件系统处理文件所需要的所有信息都存放在称为索引节点的数据结构中。主要就是**文件的属性**，包括链接数、文件所有者、文件建立和修改的时间，文件在磁盘的位置，文件大小、使用权限等等。
